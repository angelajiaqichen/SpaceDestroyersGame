...
class Player: SKSpriteNode {
  ...
  func respawn(){ // how a player regenerates if they get killed
    invincible = true // the player is made invincible during the respawn process -- while the ship is fading in and out we don't want the invaders to be able to hit them
    let fadeOutAction = SKAction.fadeOut(withDuration: 0.4) // want the respawn process to be delayed (in this case, the ship fades in and out)
